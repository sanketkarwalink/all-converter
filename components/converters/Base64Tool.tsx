...
export function Base64Tool() {
  ...
  if (!input) {
    return (
      <FileDropZone
        accept="*/*"
        label="Upload a file to encode, or paste Base64 to decode"
    ...
    <div className="space-y-4 animate-fade-in">
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          <button
            onClick={() => { setMode("encode"); setOutput(""); setError(""); if (mode !== "encode") convert("encode"); }}
            className={`tab-btn ${mode === "encode" ? "tab-btn-active" : "tab-btn-inactive"}`}
          >
            Encode
          </button>
          <button
            onClick={() => { setMode("decode"); setOutput(""); setError(""); if (mode !== "decode") convert("decode"); }}
