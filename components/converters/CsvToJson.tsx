...
export function CsvToJson() {
  ...
  const download = useCallback(() => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = fileName.replace(/\.[^.]+$/, ".json");
    a.click(); URL.revokeObjectURL(url);
    toast("JSON downloaded!", "success");
  }, [output, fileName, toast]);

  if (!input) {
    return <FileDropZone accept=".csv" label="Upload a CSV file" description="Accepts .csv files with comma-separated values" onFile={handleFile} />;
  }
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="space-y-2">
          <label className="text-sm font-medium text-zinc-600 dark:text-zinc-400">CSV Input</label>
        ...
        <div className="space-y-2">
          ...
          <textarea value={output} readOnly className="output-base h-72" placeholder="JSON output..." />
        </div>
      </div>
