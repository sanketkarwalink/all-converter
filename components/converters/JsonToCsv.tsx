...
export function JsonToCsv() {
    ...
    try {
      ...
      const csv = [
        ...
        ...data.map((row) =>
          headers
            .map((h) => {
              const str = row[h] == null ? "" : String(row[h]);
  ...
  const download = useCallback(() => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = fileName.replace(/\.[^.]+$/, ".csv");
    a.click(); URL.revokeObjectURL(url);
    toast("CSV downloaded!", "success");
  }, [output, fileName, toast]);

  if (!input) {
    return <FileDropZone accept=".json" label="Upload a JSON file" description="Accepts .json files with an array of objects" onFile={handleFile} />;
  }
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <textarea value={output} readOnly className="output-base h-72" placeholder="CSV output..." />
        </div>
      </div>
